  ...
  <page-list :search-form="searchForm"
    ...
    <template v-slot:operate-btns>
      <button-sc v-has="'production_data_base_data_model_create'"
        type="operate"
        @click="$router.push({name: 'production_data_base_data_hard_version_create'})">
        新增 +
      </button-sc>
    </template>
    <!-- 表格 -->
    <template v-slot:table>
      ...
      <el-table-column label="操作"
        ...
        fixed="right">
        <template slot-scope="scope">
          <button-tooltip-icon-operation v-has="'production_data_base_data_hard_version_delete'" type="delete" @click="deleteItem(scope.row)"></button-tooltip-icon-operation>
        </template>
      </el-table-column>
    </template>
...
<script>
import { search, deleteItem, create} from '@/api/hard-version.js';
export default {
  name: 'HardVersionIndex',
  data () {
    return {
      searchForm: { model_id: null, type_id: null, factory_id: null },
  ...
  methods: {
    ...
    deleteItem (row) {
      this.$listDeleteItem({
        ...
        row: row,
        api: deleteItem,
        message: `是否删除版本号为${row.version}的硬件版本？`,
        successMessage: `删除${row.version}成功!`,
